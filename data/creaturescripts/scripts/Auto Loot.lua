...
local function getPlayerStorageZero(cid, storage)
    local sto = getPlayerStorageValue(cid, storage)
...
local function autoLoot(cid, pos)
	...
	for i = 1, 255 do
		pos.stackpos = i
		local thingPos = getThingFromPos(pos)
		if thingPos.uid and thingPos.uid > 0 and isContainer(thingPos.uid) then
			position = pos
			check = true
			break
		end
	...
	if check then
		local corpse = getContainerItemsInfo(thingPos.uid)		
		if corpse then
...
local function autoGold(cid, pos)
	...
	if check then
		local corpse = getContainerItemsInfo(thingPos(position).uid)
		if corpse then
			for index, info in pairs(corpse) do
				if info.uid and info.itemid then
					if index < countTable(corpse) then
