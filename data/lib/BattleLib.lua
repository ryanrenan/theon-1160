_Lib_Battle_Info = {
Reward = {6549,6547,6549,5792,1,2145,1},
TeamOne = {name = "preto", storage = 140120, pos = {x=983,y=1041,z=6}},
TeamTwo = {name = "vermelho",storage = 140121,pos = {x=987,y=1041,z=6}},
...
{3517,{x=989, y=1026, z=6, stackpos = 1}},
{3517,{x=978, y=1026, z=6, stackpos = 1}},
...
str = str .. ""..#players.." jogadores"..(#players > 1 and "s" or "").." da equipe "..(getGlobalStorageValue(_Lib_Battle_Info.TeamOne.storage) == 0 and _Lib_Battle_Info.TeamTwo.name or _Lib_Battle_Info.TeamOne.name).." venceram o evento battlefield, e ganharam ".._Lib_Battle_Info.Reward[2].." "..getItemNameById(_Lib_Battle_Info.Reward[1]).." de prêmio!."
for i = 1, #players do
...
doBroadcastMessage("[Evento Battlefield] É necessário "..getGlobalStorageValue(_Lib_Battle_Info.storage_count).." jogadores, para começar!", MESSAGE_STATUS_CONSOLE_BLUE)
...
doBroadcastMessage("[Evento Battlefield] Não foi possível iniciar, porque não tinha jogadores suficientes.", MESSAGE_STATUS_CONSOLE_BLUE)
