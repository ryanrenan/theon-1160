-- These are the default replies of all npcs. They can/should be changed individually for each npc.-- These are the default replies of all npcs. They can/should be changed individually for each npc.-- These are the default replies of all npcs. They can/should be changed individually for each npc.-- These are the default replies of all npcs. They can/should be changed individually for each npc.-- These are the default replies of all npcs. They can/should be changed individually for each npc.-- These are the default replies of all npcs. They can/should be changed individually for each npc.-- These are the default replies of all npcs. They can/should be changed individually for each npc.-- These are the default replies of all npcs. They can/should be changed individually for each npc.-- These are the default replies of all npcs. They can/should be changed individually for each npc.-- These are the default replies of all npcs. They can/should be changed individually for each npc.-- Advanced NPC System (Created by Jiddo),
...
if(NpcHandler == nil) then
	...
	NpcHandler = {
		...
		messages = {
			...
			[MESSAGE_GREET] 	= 'Olá |PLAYERNAME|!, Compro diversos itens, Você tem algo para me oferecer ? diga "{negociar}" .',
			[MESSAGE_FAREWELL] 	= 'Thau |PLAYERNAME|, volte sempre.',
			[MESSAGE_BUY] 		= 'Voce quer comprar |ITEMCOUNT| |ITEMNAME| por |TOTALCOST| gps?',
			[MESSAGE_ONBUY] 	= 'Foi um prazer fazer negocio com voce.',
			[MESSAGE_BOUGHT] 	= 'Você comprou |ITEMCOUNT|x |ITEMNAME| por |TOTALCOST| gold coins.',
			[MESSAGE_SELL] 		= 'Voce quer vender |ITEMCOUNT| |ITEMNAME| por |TOTALCOST| gps?',
			[MESSAGE_ONSELL] 	= 'Obrigado por este |ITEMNAME|, |PLAYERNAME| gps.',
			[MESSAGE_SOLD]	 	= 'vendido |ITEMCOUNT|x |ITEMNAME| por |TOTALCOST| gold coins.',
			[MESSAGE_MISSINGMONEY]	= 'Desculpe, voce nao tem dinheiro suficiente.',
			[MESSAGE_NEEDMONEY] 	= 'Voce nao tem dinheiro suficiente.',
			[MESSAGE_MISSINGITEM] 	= 'Voce nao precisa nem ter esse item, |PLAYERNAME|!',
			[MESSAGE_NEEDITEM]	= '',
			[MESSAGE_NEEDSPACE]	= 'Voce não tem capacidade suficiente.',
			[MESSAGE_NEEDMORESPACE]	= 'Voce nao tem capacidade suficiente para todos os itens.',
			[MESSAGE_IDLETIMEOUT] 	= 'Em seguida, por favor!',
			[MESSAGE_WALKAWAY] 	= 'Seu mal educado, nem se despede!',
			[MESSAGE_DECLINE]	= 'Nao e bom o suficiente, é... ?',
			[MESSAGE_SENDTRADE]	= 'Aqui está minha oferta, |PLAYERNAME|. Gostou?',
			[MESSAGE_NOSHOP]	= 'Desculpe, eu nao estou oferecendo nada.',
			[MESSAGE_ONCLOSESHOP]	= 'Obrigado, volte quando voce quiser algo mais.',
			[MESSAGE_ALREADYFOCUSED]= '|PLAYERNAME|! já estou falando com você...',
			[MESSAGE_PLACEDINQUEUE] = '|PLAYERNAME|, por favor, aguarde a sua vez. tem |QUEUESIZE| clientes antes de voce.'
		}
	}
