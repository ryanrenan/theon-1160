...
[8298] = 0, -- 0% additional
[8299] = 5, -- 5%
[8301] = 10, -- 10% additional
[8302] = 15, -- 15%
[8303] = 20, -- 20% additional
}
...
return doPlayerSendTextMessage(cid, 24,"Seu item ja está no nível maximo não da mais para refina-lo.")
end
doPlayerSendTextMessage(cid, 24,"Tentando refinar com "..(chance+it[item.itemid] > 100 and 100 or chance+it[item.itemid]).."% de chance!")
if chance+it[item.itemid] >= math.random(0,100) then
local nm = getItemName(itemEx.uid)
local slot = nm:match('(%[.+%])') or '' ---If you server use slot system dont change it ^^
slot = slot~='' and ' '..slot or slot
setItemName(itemEx.uid, getItemNameById(itemEx.itemid)..' +'..(level+1)..slot)
addEvent(doPlayerSendTextMessage,500,cid, 24,"Seu item avançou para +"..(level+1)..slot..".")
doSendAnimatedText(toPosition, "REFINADO!", 30)
