local config = {
        ...
        minLevel = 20, -- Level necessário para comprar promotion.
...
function onSay(cid, words, param)
        ...
        if (config.premium == "no" and isPremium(cid)) or (config.premium ~= "no") then       
                if(getPlayerPromotionLevel(cid) >= config.promotion) then
                        ...
                        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_ORANGE, "Atualmente você já foi promovido!")
                ...
                elseif(doPlayerRemoveMoney(cid, config.cost) ~= TRUE) then
                        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_ORANGE, "Para adquirir a promotion é necessário 200 crystal coins.")
                else
